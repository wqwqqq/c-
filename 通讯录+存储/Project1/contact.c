...
void CheckCapacity(struct Contact* ps)
{
	...
	{
		struct Peoinit* ptr=(struct Peoinit*)realloc(ps->data, (ps->capacity + 2) * sizeof(struct Peoinfo));
		if (ptr != NULL)
		{
			ps->data = ptr;
			(ps->capacity) += 2;
...
void ReadContact(struct Contact* ps)
{
	...
	while (fread(&tmp, sizeof(struct Peoinfo), 1, pff))
	{
		CheckCapacity(ps);
		ps->data[ps -> size] = tmp;
		ps->size++;
